...
<script>
    export default {
        ...
        methods: {
            delTag(tag) {
                this.$store.dispatch('blog/DELETE_TAG', tag.id).then((data) => {
                    // console.log(data)
                    if (data.success) {
                        this.$refs.tip.openTip('태그를 삭제되었습니다.')
                        this.$store.dispatch('blog/TAGS').then((data) => {
                            this.tags = data.data
                        })
                    }
            ...
            edit() {
                this.isEdit = false
                this.$store.dispatch('blog/UPDATE_TAG', this.tag).then((data) => {
                    if (data.success) {
                        this.$refs.tip.openTip('태그를 수정했습니다.')
                        this.$store.dispatch('blog/TAGS').then((data) => {
                            this.tags = data.data
                        })
                    }
